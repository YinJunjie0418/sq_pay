...
import org.yeepay.core.common.util.HttpClient;
import org.yeepay.core.common.util.MyLog;
import org.yeepay.core.common.util.PayDigestUtil;
...
public class YeePayUtil {
    ...
    private static final MyLog _log = MyLog.getLog(YeePayUtil.class);
    ...
    public static Long getFee(Byte type, BigDecimal feeRate, Long feeEvery, Long amount) {
        Long fee = 0l;
        if(type == 1) {
            fee = YeePayUtil.calOrderMultiplyRate(amount, feeRate);
        }else if(type == 2) {
            fee = feeEvery;
        }
        return fee;
