...
import org.yeepay.pay.channel.TransNotifyInterface;
...
public class NotifyTransController {
	...
	public String payNotifyRes(HttpServletRequest request, @PathVariable("channel") String channel) throws ServletException, IOException {
		...
		try {
			...
			_log.error(e, "");
			return YeePayUtil.makeRetFail(YeePayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL, "转账渠道类型[channel="+channel+"]实例化异常", null, null));
		}
