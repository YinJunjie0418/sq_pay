...
import org.yeepay.pay.service.AgentpayService;
    ...
    public String queryAgentpay(HttpServletRequest request) {
        ...
        try {
            ...
            if (!"success".equalsIgnoreCase(errorMessage)) {
                ...
                return YeePayUtil.makeRetFail(YeePayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL, errorMessage, null, null));
            }
            ...
            if (mchAgentpayRecord == null) {
                return YeePayUtil.makeRetFail(YeePayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL, "代付订单不存在", null, null));
            }
            Map<String, Object> map = buildRetMap(mchAgentpayRecord);
            _log.info("###### 商户查询订单处理完成 ######");
            return YeePayUtil.makeRetData(map, payContext.getString("key"));
        }catch (Exception e) {
            _log.error(e, "");
