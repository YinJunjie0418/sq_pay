    ...
    public String buildPaymentUrl(HttpServletRequest request, @PathVariable("scene") String scene) {
        ...
        try {
            ...
            if(StringUtils.isBlank(sign)) {
                ...
                return YeePayUtil.makeRetFail(YeePayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL, errorMessage, null, PayEnum.ERR_0014.getCode(), errorMessage));
            }
            Long mchIdL;
            ...
            // 验证签名数据
            boolean verifyFlag = YeePayUtil.verifyPaySign(po, key);
            if(!verifyFlag) {
                errorMessage = "验证签名失败.";
                _log.info("{}参数校验不通过:{}", logPrefix, errorMessage);
                return YeePayUtil.makeRetFail(YeePayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL, errorMessage, null, PayEnum.ERR_0014.getCode(), errorMessage));
            ...
            return YeePayUtil.makeRetData(retObj, key);
