package org.yeepay.pay.ctrl.agentpay;
...
import org.yeepay.core.service.IYeePayAgentpayService;
...
public class AgentpayController extends BaseController {
    ...
    @Autowired
    IYeePayAgentpayService yeepayAgentpayService;
    ...
    public String transOrder(HttpServletRequest request) {
        ...
        try {
            ...
            if(mchAgentpayRecord == null) return YeePayUtil.makeRetFail(YeePayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL, "支付网关代付失败", null, null));

            int result = yeepayAgentpayService.applyAgentpay((MchInfo) agentpayContext.get("mchInfo"), mchAgentpayRecord);

            _log.info("{}创建代付订单,结果:{}", logPrefix, result);
            if(result != 1) {
                return YeePayUtil.makeRetFail(YeePayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL, "申请代付失败", null, null));
            }
            // 查询最新代付记录
            ...
            return YeePayUtil.makeRetData(map, agentpayContext.getString("key"));
        }catch (ServiceException e) {
            _log.error(e, "");
            return YeePayUtil.makeRetFail(YeePayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL, e.getErrMsg(), null, null));
        }catch (Exception e) {
            _log.error(e, "");
            return YeePayUtil.makeRetFail(YeePayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL, "支付网关系统异常", null, null));
        }
    }
    ...
    private Object validateParams(JSONObject params, JSONObject agentpayContext, HttpServletRequest request) {
        ...
        boolean isAllow = YeePayUtil.ipAllow4Strong(clintIp, mchInfo.getAgentpayWhiteIp(), mchInfo.getAgentpayBlackIp());
        if(!isAllow) {
