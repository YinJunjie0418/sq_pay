...
public class SecurityCenterController extends BaseController {
    ...
    public ResponseEntity<?> getGoogleAuthQrCode() {
        ...
        try {
            ...
            if(StringUtils.isBlank(googleAuthSecretKey)){
                ...
                if(count != 1) return ResponseEntity.ok(YeePayResponse.build(RetEnum.RET_COMM_OPERATION_FAIL));
            }
            ...
            return ResponseEntity.ok(YeePayResponse.buildSuccess(qrcodeUrl));
        }catch (Exception e) {
            _log.error(e, "生成谷歌绑定二维码异常");
            return ResponseEntity.ok(YeePayResponse.build(RetEnum.RET_COMM_OPERATION_FAIL));
        }
    }
    ...
    public ResponseEntity<?> bindGoogleAuth(HttpServletRequest request) {
        ...
        int count = rpcCommonService.rpcMchInfoService.update(updateMchInfo);
        if(count != 1) return ResponseEntity.ok(YeePayResponse.build(RetEnum.RET_MCH_GOOGLEAUTH_SECRETKEY_BIND_FAIL));
