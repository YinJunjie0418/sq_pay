package org.yeepay.manage.settlement.ctrl;
...
public class AgentpayController extends BaseController {
    ...
    public ResponseEntity<?> list(HttpServletRequest request) {
        ...
        return ResponseEntity.ok(YeePayPageRes.buildSuccess(mchAgentpayRecordList, count));
    }
    ...
    public ResponseEntity<?> get(HttpServletRequest request) {
        ...
        return ResponseEntity.ok(YeePayResponse.buildSuccess(mchAgentpayRecord));
    }
    ...
    public ResponseEntity<?> queryTrans(HttpServletRequest request) {
        ...
        JSONObject resObj = rpcCommonService.rpcyeepayTransService.queryTrans(transOrderId);
        if(YeePayUtil.isSuccess(resObj)) {
            JSONObject jsonObject = resObj.getJSONObject("channelObj");
            return ResponseEntity.ok(YeePayResponse.buildSuccess(jsonObject == null ? "转账接口没有返回channelObj" : jsonObject.toJSONString()));
        }else {
            return ResponseEntity.ok(YeePayResponse.buildSuccess("查询通道异常"));
        }
    }
