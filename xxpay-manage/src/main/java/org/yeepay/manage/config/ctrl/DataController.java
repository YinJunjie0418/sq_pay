...
public class DataController extends BaseController {
    ...
    public ResponseEntity<?> count4mchTop(HttpServletRequest request) {
        ...
        return ResponseEntity.ok(YeePayResponse.buildSuccess(mchTopList2));
    }
    ...
    public ResponseEntity<?> count4AgentTop(HttpServletRequest request) {
        ...
        return ResponseEntity.ok(YeePayResponse.buildSuccess(agentTopList2));
    }
    ...
    public ResponseEntity<?> count4PayProduct(HttpServletRequest request) {
        ...
        return ResponseEntity.ok(YeePayResponse.buildSuccess(payProductList2));
    }

    @RequestMapping("/data/selectMchInfo")
    @ResponseBody
    public YeePayResponse selectMchInfo(HttpServletRequest request) {
        JSONObject param = getJsonParam(request);
        Long mchId = getLong(param, "mchId");
        if (mchId != null) {
            MchInfo mchInfo = rpcCommonService.rpcMchInfoService.findByMchId(mchId);
            if (mchInfo == null) { return YeePayResponse.build(RetEnum.RET_SERVICE_MCH_NOT_EXIST); }
        }
        return YeePayResponse.buildSuccess();
    }
    ...
    public ResponseEntity<?> selectSuccessRate(HttpServletRequest request) {
        ...
        return ResponseEntity.ok(YeePayPageRes.buildSuccess(successRateList2,count));
    }
