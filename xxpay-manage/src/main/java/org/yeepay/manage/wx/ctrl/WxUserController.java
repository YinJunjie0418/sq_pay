package org.yeepay.manage.wx.ctrl;
...
import org.yeepay.core.entity.WxUser;
import org.yeepay.manage.common.config.MainConfig;
import org.yeepay.manage.common.ctrl.BaseController;
import org.yeepay.manage.common.service.RpcCommonService;
import org.yeepay.manage.wx.service.WxUserService;
...
public class WxUserController extends BaseController {
    ...
    public ResponseEntity<?> get(HttpServletRequest request) {
        ...
        return ResponseEntity.ok(YeePayResponse.buildSuccess(wxUser));
    }
    ...
    public ResponseEntity<?> update(HttpServletRequest request) {
        ...
        if(dbWxUser == null) {
            ResponseEntity.ok(YeePayResponse.build(RetEnum.RET_COMM_RECORD_NOT_EXIST));
        }
    ...
    public ResponseEntity<?> list(HttpServletRequest request) {
        ...
        return ResponseEntity.ok(YeePayPageRes.buildSuccess(wxUserList, count));
    }
    ...
    public ResponseEntity<?> qrLogin(HttpServletRequest request) {
        ...
        Long userId = getLongRequired(param, "userId");
        WxUser wxUser = rpcCommonService.rpcWxUserService.findByUserId(userId);
        if(wxUser == null) return ResponseEntity.ok(YeePayResponse.build(RetEnum.RET_MGR_WX_USER_NOT_EXIST));

        // 调用微信API服务端,得到该用户登录二维码
        JSONObject resObj = wxUserService.geLoginQrcode(wxUser);
        if(resObj == null) return ResponseEntity.ok(YeePayResponse.build(RetEnum.RET_MGR_WX_USER_LOGIN_FAIL));
        if("0".equals(resObj.getString("code"))) {
            return ResponseEntity.ok(YeePayResponse.buildSuccess(resObj.getJSONObject("data")));
        }
    ...
    public ResponseEntity<?> againLogin(HttpServletRequest request) {
        ...
        if(resObj == null) return ResponseEntity.ok(YeePayResponse.build(RetEnum.RET_MGR_WX_API_RETURN_FAIL));
        if("0".equals(resObj.getString("code"))) {
            return ResponseEntity.ok(YeePayResponse.buildSuccess(resObj.getJSONObject("data")));
        }
        return ResponseEntity.ok(YeePayResponse.buildSuccess());
    ...
    public ResponseEntity<?> logout(HttpServletRequest request) {
        ...
        if(resObj == null) return ResponseEntity.ok(YeePayResponse.build(RetEnum.RET_MGR_WX_USER_LOGOUT_FAIL));
        if("0".equals(resObj.getString("code"))) {
            return ResponseEntity.ok(YeePayResponse.buildSuccess(resObj.getJSONObject("data")));
        }
        return ResponseEntity.ok(YeePayResponse.buildSuccess());
    ...
    public ResponseEntity<?> getPaymentQrcode(HttpServletRequest request) {
        ...
        if(wxUser.getLoginStatus() != 3) {
            return ResponseEntity.ok(YeePayResponse.build(RetEnum.RET_MGR_WX_USER_NOT_LOGIN));
        }
        ...
        if("0".equals(resObj.getString("code"))) {
            ...
            if(StringUtils.isEmpty(payUrl)) {
                return ResponseEntity.ok(YeePayResponse.build(RetEnum.RET_MGR_WX_USER_LOGIN_FAIL));
            }
