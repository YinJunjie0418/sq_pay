package org.yeepay.manage.wx.service;
...
public class WxUserService {
    ...
    public JSONObject geLoginQrcode(WxUser wxUser) {
        ...
        loginUrl += "?" + YeePayUtil.genUrlParams(paramMap);
        _log.info("发起微信登录请求,url={}", loginUrl);
        String result = YeePayUtil.call4Post(loginUrl);
    ...
    public JSONObject getPaymentQrcode(WxUser wxUser) {
        ...
        paymentUrl += "?" + YeePayUtil.genUrlParams(paramMap);
        _log.info("发起微信获取收款二维码请求,url={}", paymentUrl);
        String result = YeePayUtil.call4Post(paymentUrl);
        if(StringUtils.isEmpty(result)) return null;
        return JSON.parseObject(result);
    }
